...
class Widgets extends Module implements EventSubscriberInterface
{
    ...
    public function displayWidget($instance, $widget, $args)
    {
        ...
        $this->widgets[$this->sidebar][] = $this->createWidget([
            ...
            'config' => (new Collection($this->config['defaults']))->merge($config),
        ]);
    ...
    public function editScreen($screen)
    {
        if (in_array($screen->base, ['customize', 'widgets'])) {
            $this->scripts
                ->add('widgets', "{$this->path}/app/widgets.min.js", ['uikit', 'commons', 'widgets-data'])
    ...
    {
        static $id = 0;
        return (object) array_merge(['id' => 'tm-'.(++$id), 'title' => '', 'position' => $this->sidebar, 'attrs' => ['class' => []], 'config' => new Collection($this->config['defaults'])], (array) $widget);
    }
